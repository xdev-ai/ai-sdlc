...
| [`docs/telemetry-configuration-and-trace-context.md`](telemetry-configuration-and-trace-context.md) | Sprint 1 implementation: versioned `aisdlc.telemetry` configuration model, resource/span/metric allowlists, and W3C trace-context propagation in the management server. | Covered by `TelemetryPropertiesTest`, `TelemetryAttributeContractTest`, `W3CTraceContextTest`, and `TraceContextFilterTest`; telemetry stays disabled by default. |
