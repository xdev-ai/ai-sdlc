...
`.trivyignore.yaml` is absent by default and must be created only when an accepted-risk exception is approved. Each entry must identify the advisory, affected package or artifact, business/technical rationale, approving reviewer, accountable owner, approval date, and an explicit expiry date. The exception must be reviewed before expiry, removed as soon as remediation is available, and never used to suppress a scanner-operational failure. Suppression does not delete SARIF evidence or remove the original finding from audit history.
