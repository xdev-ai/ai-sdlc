...
- [x] P3.1.1: define the OpenTelemetry resource model, semantic-convention profile, trace-propagation contract, and telemetry data-classification policy. (TelemetryProperties, TelemetryAttributeContract, W3CTraceContext, TraceContextFilter. See docs/telemetry-configuration-and-trace-context.md.)
- [x] P3.1.2: instrument Spring MVC, Spring Security, JPA, outbound HTTP, object evidence storage, notification delivery, policy evaluation, and SCM ingestion with OpenTelemetry. (GovernanceTelemetry spans across policy evaluation, approvals, evidence, SCM ingestion, notification delivery, and the audit ledger. See docs/telemetry-agent-and-instrumentation.md.)
...
- [x] P3.1.5: add alert-routing, incident linkage, SLO dashboards, burn-rate rules, operational audit evidence, and synthetic health checks. (p3-slo-burn-rate-rules.yaml, alertmanager-routes.yaml, docs/slo-runbooks.md, scripts/synthetic-health-journey.sh.)
- [x] P3.1.6: add telemetry contract tests, cardinality/privacy regression tests, collector resilience tests, and SLO runbooks. (Telemetry contract and cardinality tests, ResilienceScenarioTest collector cases, p3-slo-rule-tests.yaml, and docs/slo-runbooks.md.)
- [x] P3.3.1: define the runtime AI governance gateway threat model, trust boundaries, decision taxonomy, data classification, and tenant-isolation requirements. (docs/p3-reliability-and-runtime-ai-governance-design.md, threat model and decision taxonomy.)
...
- [x] P3.1 Resilience: define controlled chaos game days, blast-radius guardrails, abort criteria, evidence capture, recovery SLOs, and operator runbooks. (docs/p3-1-resilience-chaos-test-plan.md section 1 blast radius, abort criteria, and evidence fields; section 4 game-day procedure; section 5 tier acceptance. Operator runbooks in docs/slo-runbooks.md.)
...
- [x] P3.2: define a tenant-safe inference usage ledger, canonical price catalog, currency normalization, data-retention model, and cost attribution dimensions. (docs/p3-2-cost-optimization-and-inference-forecasting-design.md sections 3, 4, and 7; implemented by V14 inference_usage_events, inference_cost_allocations, and InferenceCostService with ISO currency minor units.)
...
- [x] P3.2: design provider adapters for usage reconciliation and model-routing recommendations that remain advisory until an explicit human decision. (docs/p3-2-cost-optimization-and-inference-forecasting-design.md section 2 UsageIngestionAdapter and PriceCatalogAdapter boundaries, and section 6 reconciliation-gap handling. Recommendations stay advisory: no forecast or budget decision routes a model.)
...
- [x] P3.3 Implementation: add controller/API integration, service-level regression tests, and English documentation for runtime AI enforcement rollout and recovery. (Four runtime AI controllers with tests, plus docs/ai-agent-governance.md, docs/runtime-ai-workload-identity.md, docs/runtime-ai-tool-broker.md, and docs/p3-provider-proxy-execution-design.md.)
