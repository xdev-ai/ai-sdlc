...
class SecurityConfigTest {
  ...
  void mapsOnlySupportedKeycloakRealmRoles() {
    Jwt jwt = Jwt.withTokenValue("token").header("alg", "none").claim("sub", "subject-1").claim("roles", List.of("developer", "untrusted", "reviewer")).issuedAt(Instant.now()).expiresAt(Instant.now().plusSeconds(60)).build();
