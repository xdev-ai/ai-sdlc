...
public class RuntimeAiBrokerController {
  ...
  record ProviderInput(@NotBlank @Size(max=160) String provider, @NotBlank @Size(max=240) String model, @NotNull UUID policyBundleId, @NotBlank @Size(max=240) String credentialReference, @Min(100) @Max(120000) int timeoutMs, @Min(1) @Max(3) int maxAttempts, boolean active) {}
  ...
  @PutMapping("/providers") @ResponseStatus(HttpStatus.NO_CONTENT) public void configureProvider(@PathVariable UUID projectId,@RequestBody @Valid ProviderInput input,@AuthenticationPrincipal Jwt jwt){broker.configureProvider(projectId,jwt.getSubject(),input.provider(),input.model(),input.policyBundleId(),input.credentialReference(),input.timeoutMs(),input.maxAttempts(),input.active());}
