...
- **The last snake_case keys reaching a client were inside a risk snapshot.** `sourceSummary.latestQuality` embedded a raw `quality_metric_snapshots` row in an otherwise typed response, so nine database column names went out as JSON fields. They are aliased now. Snapshots written before this change keep the old keys, because the map is stored as JSON at calculation time and is not rewritten.
...
- P3.5 connector expansion (inbound): a versioned `scm.inbound.v1` connector contract with GitLab, Bitbucket, Azure DevOps, and Jira implementations, a provider-neutral `/api/v1/webhooks/scm/{connector}` ingress that verifies before parsing, per-connector credential modes with configurable header names, deterministic delivery identifiers for providers that send none, `WORK_ITEM` event support for work-management providers, and a contract test suite run against all four. Outbound policy feedback for the new connectors is not included.
- P3.4 policy-pack catalog: a signed, versioned catalog with lifecycle states, compatibility metadata, digest-pinned distribution channels, published-version immutability enforced against git history, keyless Cosign release signing over a checksum manifest, and a promotion workflow that opens a reviewed pull request instead of pushing. Delivered in the `xdev-ai/ai-sdlc-policies` repository.
- P3.4 enterprise deployment package: a Helm chart for the two stateless workloads with hardened defaults (non-root, read-only root filesystem, all capabilities dropped, no service-account token, default-deny network policy, ClusterIP-only management API, PodDisruptionBudgets, resource limits), render-time refusal of mutable image tags and of telemetry without an exporter endpoint, no chart-managed Secret, Argo CD and Flux GitOps references pinned to a tag, upgrade and rollback guidance covering the forward-only migration limit, and a CI hardening contract test.
- P3.1 Collector deployment, SLO definitions, and alerting: the hardened Collector gateway wired into Docker Compose behind an opt-in `observability` profile, versioned SLO target ratios split into error-budget and integrity policies, burn-rate alerting restricted to error-budget objectives so an integrity target of 1.0 cannot divide by zero, per-journey runbooks and alert routing with inhibition, audit and evidence integrity counters, an authenticated synthetic health journey, promtool rule unit tests, offline Collector resilience and cardinality contract tests, and an isolated chaos-tier CI job with static profile-isolation guardrails.
